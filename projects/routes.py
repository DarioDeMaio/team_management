...
def edit_project(project_id):
    if request.method == "POST":
        ...
        for member_id in updated_members:
            user = users.find_one({"_id": member_id})
            if user:
                if 'Projects' in user and ObjectId(project_id) not in user['Projects']:
                    user['Projects'].append(ObjectId(project_id))
                else:
                    user['Projects'] = [ObjectId(project_id)]
                users.update_one({"_id": member_id}, {"$set": {"Projects": user['Projects']}})
        
        current_project = projects.find_one({"_id": ObjectId(project_id)})
        current_members = [ObjectId(member_id) for member_id in current_project.get('Members', [])]
        
        for member_id in current_members:
            if member_id not in updated_members:
                result = projects.update_one(
                    {"_id": member_id},
                    {"$pull": {'Members': ObjectId(project_id)}}
                )
                print(f"Removing project {project_id} from user {member_id}. Matched: {result.matched_count}, Modified: {result.modified_count}")
        
        return redirect(url_for('projects_bp.get_projects'))
    
    user_list = list(users.find())
    project = projects.find_one({"_id": ObjectId(project_id)})
    return render_template("projects/edit_project.html", user_list=user_list, project=project)
